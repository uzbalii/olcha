...
<FaRegHeart className='icon_heart1' />
        <BiBarChart className='icon_heart' />
      <h2>{name}</h2>
      <p >{price.toLocaleString()} so‘m</p>
      <div>855 000 som x 12 oy</div>
    </div>
    <div className="btn">
          <PiShoppingCartSimpleBold className='card_icon' /> <button className='btn6'>Muddatli tolov</button>
    </div>
  ...
  return (
    <div className="product">
      {smartfonlar.map((item, index) => (
        <Card key={index} name={item.name} price={item.price} image={item.images[0]} />
      ))}
    </div>  
  );
};
